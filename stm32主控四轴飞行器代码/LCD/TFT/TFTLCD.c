...
u16 yi=0;
u8 i=0;
struct zuobiao zuobiao1={30,230,0,0};	
// 	
	 u16 sum_time;			 
//画笔颜色,背景颜色
...
{										   
	u16 t;
 	GPIOB->CRL=0X88888888; //PE0-7  上拉输入
	GPIOB->CRH=0X88888888; //PE8-15 上拉输入
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,' ',size,0);//len大于数值的实际长度，则在最前面打空格
				continue;
			}else enshow=1; 
...
//size:字体大小
//num:数值	
void LCD_ShowDec(u16 x,u16 y,float num,u8 size)
{         	
	u8 t,temp;
	u8 enshow=0;
	u16 zhengshu=0;
	...
	u16 x2,x3;
	zhengshu=(u16)num;
	xiaoshu=((num-zhengshu));//越大精度越高,r如果小数是0，则值输出一个0
	//整数部分
	for(t=0;t<length(zhengshu);t++)
	{
		temp=(zhengshu/mypow(10,length(zhengshu)-t-1))%10;
		if(enshow==0&&t<(length(zhengshu)-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,' ',size,0);////len大于数值的实际长度，则在最前面打空格
	...
	//计入小数部分的起始位置
		...
		LCD_ShowChar(x+(size/2)*(t),y,'.',size,0); //小数点
	x2=x+(size/2)*(t+1);
	//小数部分（如0.0034）
	xiaoshu2=(u16)(xiaoshu*10);
	t=0;
	while(xiaoshu2==0)
	{
		LCD_ShowChar(x2+(size/2)*t,y,'0',size,0); 
		xiaoshu2=(u16)(xiaoshu*mypow(10,t+2));
		t++;
		if(t>=2)//修改3可改变可判别的0 的个数
			break;
	}
	//////小数部分(取整部分)
	x3=x2+(size/2)*(t);
	xiaoshu2=((num-zhengshu)*100);//越大精度越高,r如果小数是0，则值输出一个0
	for(t=0;t<length(xiaoshu2);t++)
	{
		temp=(xiaoshu2/mypow(10,length(xiaoshu2)-t-1))%10;

	 	LCD_ShowChar(x3+(size/2)*t,y,temp+'0',size,0); 
	}
} 
/***********************************画曲线坐标********************/
//flag=0  y方向上写作标值，1  x方向上写作标值
//unit 最小单位值
//dual 分成几段
//len  数值的长度
//x1,y1 起始坐标,x1至少大于8
//默认用12字体
void set_axes(u16 x1,u16 y1,u16 x2,u16 y2,u8 flag,u8 unit,u8 dual,u8 len)
{
	u8 i=0;
	u16 space=0;
	...
	if(flag==0)//y方向上写作标值
	{
		...
		for(i=0;i<=dual;i++)
		{
			LCD_ShowNum(0,y2-space*i,0+unit*i,len,12);
		}
	}
		...
		if(flag==1)//x方向上写坐标值
	{
		...
		for(i=0;i<=dual;i++)
		{
			LCD_ShowNum(space*i,300,0+unit*i,len,12);
		}
	}
}
/***************************************************************************/
...
void lCD_DrawCurve( u16 x1,u16 y1,u16 x2,u16 y2,u16 unit,u16 dual,u8 len,const float val)
{
	static u16 x0=30;
	u16 space=0;
	float dian_chazhi=0;
	...
	space=*p/dual;
	dian_chazhi=(float)unit/space;
	zuobiao1.now_y=y2-(int)(val/dian_chazhi);//调节6 可以使显示的值适应坐标值,增大24往下移，减小24向上移
																								//，每个坐标值纵坐标占了12，所以往下移
	zuobiao1.now_x=x0;
	if(i==0)
	{
		zuobiao1.last_y=zuobiao1.now_y;
		zuobiao1.now_x=zuobiao1.now_x;
		i++;
	}	
	//LCD_DrawPoint(zuobiao1.now_x,zuobiao1.now_y);	
		LCD_DrawLine(zuobiao1.last_x,zuobiao1.last_y,zuobiao1.now_x,zuobiao1.now_y);
		zuobiao1.last_y=zuobiao1.now_y;
		zuobiao1.last_x=x0++;
...
//描点 
void Draw_a_Big_Point(u8 x,u16 y)
{	    
...
//小数显示
//支持16大小的字体
void LCD_float(u8 x1,u8 y1,u8 sum_int,u8 sum_float,const float data)//const 保护数据
{
	u16 data_int=0,data_float=0;
	data_int=(u16)data;//整数部分
	data_float=(u16)(data*jieceng(sum_float));
	data_float=data_float%jieceng(sum_float);//小数部分
	
	LCD_ShowNum(x1,y1,data_int,sum_int,16);
	LCD_ShowChar(x1+8*sum_int,y1,'.',16,0);//显示一个字符
	LCD_ShowNum(x1+8*sum_int+8,y1,data_float,sum_float,16);
	
}

/***********把数据转换成16进制********************/
//默认16格式
void LCD_16Show(u8 x,u8 y,const u8 data)
{
	...
	if(data_h<=9)
	{
	 LCD_ShowNum(x,y,data_h,1,16);
	}
	else
	{
		LCD_ShowChar(x,y,'A'+data_h-10,16,0); 
	}
	if(data_l<=9)
	{
	  LCD_ShowNum(x+8,y,data_l,1,16);
	}
	else
	{
	 LCD_ShowChar(x+8,y,'A'+data_l-10,16,0); 
	}
}
