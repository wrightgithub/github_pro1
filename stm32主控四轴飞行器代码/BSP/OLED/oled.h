...
#define OLED_DC_OH		GPIO_SetBits(OLED_GPIO, OLED_DC) 	//DC IO口输出高电平
  ...
  void OLED_PutPixel(unsigned char x,unsigned char y,unsigned char t);
...
void Dis_Float(unsigned char Y,unsigned char X,const float real,unsigned char N); 
  ...
  void OLED_Num5(unsigned char x,unsigned char y,unsigned int number);
	void OLED_Num7(unsigned char x,unsigned char y,unsigned long number);
	void Draw_Pic(u8 *data);
	u32 OLED_mypow(u8 m,u8 n);
	void OLED_ShowNum(u8 x,u8 y,const u32 num,u8 len,u8 size);
#endif
