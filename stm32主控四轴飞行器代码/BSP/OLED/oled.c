...
//功能描述：绘制一个点（x,y）
//参数：真实坐标值(x,y),x的范围0～127，y的范围0～64,t:0或1
//返回：无
//==============================================================
...
{
        unsigned char  baiwan ,shiwan ,wan,qian,bai,shi,ge;
	baiwan =number/1000000;
	number=number%1000000;
	
	shiwan=number/100000;
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				Dis_Char(x+(size/2)*t,y,' ');
				continue;
			}else enshow=1; 
		 	 
		}
	 	Dis_Char(x+(size/2)*t,y,temp+'0'); 
	}
} 
...
//		  for(k=0;k<8;k++)
//			{
//			  ret=data[i*10+j]&0x01;
//				if(ret==1)
//				{
//				  OLED_PutPixel(j*8+k,i,0x01);
//				}
//				else
//				{
//				  OLED_PutPixel(j*8+k,i,0x00);
//				}
//			}
//		}
//	}
